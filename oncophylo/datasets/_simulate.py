...
def simulate(n_cells = 5,
    ...
    # set types
    T.graph["type"] = CONST.MUTATION_TREE
    Tc.graph["type"] = "copy_number_tree"
    T.graph["root_id"] = 0
    ...
    # cell tree
    celltree = T.copy()
    celltree.graph["type"] = CONST.CELL_TREE
    ...
    for cell in range(n_cells):
        for mutation in range(n_mutations):
            ...
            variant_read_threshold = np.ceil(nreads*vaf_threshold)
    ...
    df_B = pd.DataFrame(B, index=["root"] + event_order,
                        columns = [f'm{idx}' for idx in range(n_mutations)] + [CONST.CLUSTER_ID], dtype=int)            
    ...
    to_return = {CONST.OBS_DATA: df_Acell_noisy, 
                 CONST.TRUE_DATA: df_Acell, 
                 CONST.CELL_TREE: celltree, 
                 CONST.CLONAL_TREE: to_clonal_tree(celltree, df_Acell.drop(columns="cluster_id")),
                 CONST.MUTATION_TREE: T,
                 CONST.FPR: fpr,
                 CONST.FNR: fnr}
    
    if return_reads:
        to_return[CONST.NOISE_FREE_COUNTS] = {CONST.ALT_READS: df_Vcount, CONST.TOTAL_READS: df_Rtotal}
        to_return[CONST.NOISY_COUNTS] = {CONST.ALT_READS: df_Vcount_missing, CONST.TOTAL_READS: df_Rtotal_missing}
    if return_cntree:
        to_return[CONST.CN_TREE] = Tc
