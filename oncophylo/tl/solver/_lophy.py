...
def generate_restart_seeds(seed, num_restarts):
    """"Helper function to generate a series of seeds for each MCMC chain"""
...
def _run_chain(args):
    """Helper function to run an instance of the phylospace algorithm"""
    chain_id, chain_seed, kwargs = args

    kwargs = kwargs.copy()
    kwargs["seed"] = int(chain_seed)

    return _lophy(**kwargs)
    
def LoPhy(character_matrix, 
          ...
          return_best_solution_only=True,
          destination_dir=os.path.join(os.path.abspath(""), "LoPhy_output")):
    """
    LoPhy Python Wrapper
    ...
    psi: float
        A likelihood penalty multiplier for adding/removing CNAs
    seed: int
    ...
    remove_temp_dir: bool
        Flag to remove the temporary directory used to store LoPhy's input/output files (Default = True)
    return_best_solution_only: bool
        Flag to return only the best solution (Default = True)
    destination_dir: str
    ...
    """
    assert num_restarts > 0, "num_restarts must be a positive integer"
    assert max_cn in [2, 3, 4, 5], "max_cn must be one of the following values: 2, 3, 4, or 5"

    # make sure we have a numeric seed to name temporary paths
    if seed is None:
        seed = np.random.randint(0, np.iinfo(np.int32).max)

    kwargs = {
        "character_matrix": character_matrix,
        "variant_reads_df": variant_reads_df,
        "total_reads_df": total_reads_df,
        "region_reads_df": region_reads_df,
        "meta_df": meta_df,
    ...
    # run a single chain without the multiprocessing overhead
    if num_restarts == 1:
        results = [_lophy(seed=seed, **kwargs)]
    else:
        # obtain seeds for each run
        chain_seeds = generate_restart_seeds(seed, num_restarts)

        worker_args = [
            (i, chain_seeds[i], kwargs)
            for i in range(num_restarts)
        ]
        # run each instance of the LoPhy algorithm in parallel using multiprocessing
        with Pool(processes=num_restarts) as pool:
            results = pool.map(_run_chain, worker_args)

    results.sort(
        key=lambda x: float(x[op.ul.DATA.CELL_TREE].graph["llh"]),
        reverse=True,
    )

    # rewrite output file names in order of descending log-likelihood
    for i, result in enumerate(results):
        run_seed = int(result[op.ul.DATA.CELL_TREE].graph["seed"])
        source_file = os.path.join(destination_dir, f"out_{run_seed}.gv")
        destination_file = os.path.join(destination_dir, f"out_ml{i}.gv")
        shutil.move(source_file, destination_file)

    if num_restarts == 1 or return_best_solution_only:
        return results[0]
    else:
        return results


def _lophy(character_matrix, 
           variant_reads_df,
           total_reads_df,
           region_reads_df,
           meta_df,
    ...
    list or dictionary
        A dictionary or a list of dictionaries containing solutions and their corresponding meta data
    """
    temp_path = os.path.join(os.path.abspath(""),f"LoPhy_temp_{seed}")
    ...
    if sample_agnostic:
        args.append("-sample-agnostic")

    # run scOrchard
    output, time = op.ul.subprocess([op.ul.binary_path("LoPhy")] + args)
    ...
    dot_fn = os.path.join(output_path, output_prefix + f"_{seed}.gv")
